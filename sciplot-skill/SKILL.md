...
description: >
  ...
  **只要用户对话中出现以下任何内容，立即触发本技能**：
  
  ### 核心触发词（100%触发）
  - 画图、出图、作图、绘图、可视化、图表、图形、图像
  - figure、plot、chart、graph、diagram、visualization
  - matplotlib、seaborn、plotly、pandas.plot、pyplot
  - 论文插图、论文配图、期刊图、学术图、科研图
  - 竞赛图、比赛图、答辩图、汇报图、展示图
  - 结果图、数据图、分析图、统计图、实验图
  
  ### 图表类型触发词（100%触发）
  - 折线图、散点图、柱状图、条形图、饼图、面积图
  - 箱线图、小提琴图、热力图、等高线图、雷达图
  - 直方图、密度图、误差条图、置信区间图
  - 时间序列图、趋势图、对比图、分布图
  - 分组柱状图、堆叠柱状图、水平柱状图
  - 3D图、曲面图、线框图、3D散点图
  - 网络图、关系图、树状图、层次图、聚类图
  - 混淆矩阵、特征重要性图、学习曲线、ROC曲线
  - PCA图、降维图、平行坐标图、维恩图
  ...
  ### 隐含触发场景（90%触发）
  - 用户提供了数据（数组、列表、DataFrame）并询问如何处理
  - 用户描述了实验结果并询问如何展示
  - 用户提到了统计指标并询问如何呈现
  - 用户询问 matplotlib/seaborn/plotly 的使用方法
  - 用户需要美化现有图表
  - 用户需要调整图表样式、配色、布局
  
  ## 不触发的情况（极少数）
  - 用户明确要求使用其他特定库（如"用 plotly 画交互式图"）
  - 用户需要动画、视频、地图、交互式仪表板
  - 用户明确说"不要用 SciPlot"
  ...
  - 全类型图表：折线、散点、柱状、分组/堆叠/水平柱状、面积、箱线、小提琴、热力图、阶梯图、误差条、置信区间、组合图、雷达图、时间序列、平行坐标、网络图、层次聚类、维恩图、3D图
  ...
  ## 使用方式
  直接 `import sciplot as sp`，无需复制任何文件到项目目录。所有图表代码必须生成为独立可运行的 Python 脚本。
---

## 快速决策

**默认策略：只要和画图、图表、可视化、数据展示有关，立即使用本技能。**
